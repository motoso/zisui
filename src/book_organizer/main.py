...
class BookOrganizer:
    ...
    def analyze_files(self) -> Dict[str, Dict]:
        ...
        for title in file_groups:
            file_groups[title]['numbered'].sort(key=lambda x: self._natural_sort_key(x.name))
        
        # 空のグループを除去
        return {title: files for title, files in file_groups.items() 
